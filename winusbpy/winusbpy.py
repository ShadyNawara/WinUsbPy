...
class WinUsbPy(object):
    ...
    def list_usb_devices(self, **kwargs):
        ...
        while self.api.exec_function_setupapi(
            ...
            ret = self.api.exec_function_setupapi(
                CM_Get_Parent, byref(dev_inst_parent), getattr(sp_device_info_data, "dev_inst"), 0
            )
            if not ret:
                ...
                if not ret:
                    parent_path = buff_parent_path.value
            ...
            self.device_paths.append(WinUsbDevice(path, name, parent_path))
            i += 1
            member_index = DWORD(i)
    ...
    def extract_device_from_vid_pid(self, vid, pid, dev_list):
        ...
        for dev in dev_list:
            dev_path = dev.path
            if (
                dev_path.find("vid_{:04x}&pid_{:04x}".format(int(str(vid), 16), int(str(pid), 16)))
                != -1
            ):
                # dev found
                name = dev.name
    ...
    def init_winusb_device_with_path(self, path):
        ...
        result = self.api.exec_function_winusb(
            WinUsb_Initialize, self.handle_file, byref(self.handle_winusb)
        )
        if result == 0:
    ...
    def query_device_info(self, query=1):
        ...
        result = self.api.exec_function_winusb(
            WinUsb_QueryDeviceInformation, self.handle_winusb, info_type, byref(buff_length), buff
        )
        if result != 0:
            return buff[0]
        else:
    ...
    def query_pipe(self, pipe_index):
        pipe_info = PipeInfo()
        result = self.api.exec_function_winusb(
            WinUsb_QueryPipe, self.handle_winusb, c_ubyte(0), pipe_index, byref(pipe_info)
        )
        if result != 0:
            return pipe_info
        else:
    ...
    def flush(self, pipe_id):
        result = self.api.exec_function_winusb(
            WinUsb_FlushPipe, self.handle_winusb, c_ubyte(pipe_id)
        )
        return result
    ...
    def overlapped_read(self, pipe_id):
        ...
        rl = c_ulong(0)
        result = self.api.exec_function_winusb(
            WinUsb_GetOverlappedResult, self.handle_winusb, byref(self.olread_ol), byref(rl), False
        )
        if result == 0:
            if (
